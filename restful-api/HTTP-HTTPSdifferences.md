...
Headers: Key-value pairs with additional info (e.g., Host, User-Agent).

Host: www.example.com
User-Agent: Mozilla/5.0 (Windows NT 10.0)
Message Body (Optional): Data sent to the server (e.g., form data for POST).
...
HTTP/1.1 200 OK
Headers: Key-value pairs with response info (e.g., Content-Type, Date).

Content-Type: text/html; charset=UTF-8
Content-Length: 1234
Message Body (Optional): The requested resource or error message.
